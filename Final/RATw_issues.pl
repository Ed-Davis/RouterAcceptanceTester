...
my ($uptime, $pid, $lastpid, $passkey, $rspage, $response, $nextstuff,
	...
	$fwstart, $modstart, $rfw, $lanport, $macstart, @lines, $line, $file, @files, $p, $host, $pass_or_fail, $router_gui,
	$wantest, $ssid, $model, $proto, $value, $supplicant_file, @wifiresults, $routername) = ();
my $return_state=(0);
...
sub FTP_Results	{
			use Net::FTP;
			my $ftp = Net::FTP->new('cheesywotsit.dyndns.org:41102', Passive => 1) or die complete();
			$ftp->login('beaglerat','RT41102Beagle') or die complete();
			$ftp->cwd("RAT_Results") ;
				@files=</RAT/Reports/*>;
				foreach $file(@files)	{
				$ftp->put($file);
...
timeout $timeout => sub		{
		...
		while (1)			{
			...
			my $ping=Net::Ping->new("$proto", 0.5);
...
sleep (3);
goto INITIALIZE;  ### If it exits a sub - restart script (same $pid so wont help a crash)
